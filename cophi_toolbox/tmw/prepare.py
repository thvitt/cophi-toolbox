...
def read_tei5(teiPath, txtFolder, xpath):
    ...
    Todo:
        * do we need :func:`lxml.etree.strip_tags` at all?
          If so, make configurable & sanitize with `xpath` option
        * :func:`lxml.etree.strip_elements` stuff should be made configurable
    ...
    for file in glob.glob(teiPath):
        with open(file, "r"):
            ...
            etree.strip_elements(
                xml, "{http://www.tei-c.org/ns/1.0}note", with_tail=False)
            etree.strip_elements(
                xml, "{http://www.tei-c.org/ns/1.0}quote", with_tail=False)
            # etree.strip_elements(xml, "{http://www.tei-c.org/ns/1.0}l",
            # with_tail=False)
            # etree.strip_elements(xml, "{http://www.tei-c.org/ns/1.0}p",
            # with_tail=False)
            etree.strip_elements(
                xml, "{http://www.tei-c.org/ns/1.0}head", with_tail=False)
            # etree.strip_elements(xml, "{http://www.tei-c.org/ns/1.0}stage",
            # with_tail=False)
            etree.strip_elements(
                xml, "{http://www.tei-c.org/ns/1.0}speaker", with_tail=False)
...
def writesegment(segment, outfolder, filename, counter, mode="w"):
    ...
    Args:
        ...
        outfolder (str): Path to a folder where to write the segment files.
...
def write(segment, file, mode="w"):
    """
    subfunction of writesegment(segment, outfolder, filename, target,
    tolerancefactor, preserveparagraphs) writes the segment to a file.

    Args:
        segment(List): A list containing all words of the segment.
        file: filename of the file to be written consisting of the segment's
        origin filename + a number extension
        mode = "w": mode for the write function
...
def writesegment(segment, outfolder, filename, target, tolerancefactor, preserveparagraphs):
    ...
    Args:
        segment (List): A list containing all words (tokenized) of one line of
        an input text.
    ...
    Todo:
        ...
        *if segment == ["\n"] or len(segment) < 1: there is probably a more
    ...
    # workaround for easy inter line-spacing in case of paragraph removal for
    # lines combined into one segment
    ...
    if currentsegmentsize * tolerancefactor < target:
        # min size limit not reached => split
        # split segment
        wordsliceindex = target - currentsegmentsize

        # if it's too big: slice!
        if currentsegmentsize + len(segment) > target * tolerancefactor:
            # print(relname + "\t Last segment size: " +
            # str(currentsegmentsize) + "\t appending " + str(wordsliceindex) +
            # "\t for a total of " + str((currentsegmentsize + wordsliceindex)))
            ...
            counter += 1
            currentsegmentsize = 0
            segname = join(outfolder, filename +
                           "§{:04d}".format(counter) + ".txt")
            relname = filename + "§{:04d}".format(counter) + ".txt"
            if os.path.isfile(segname):
                os.remove(segname)
        # else just add text to current segment
        else:
            # print(relname + "\t Last segment size: " + str(currentsegmentsize)
            # + "\t appending " + str(len(segment)) + "\t for a total of " +
            # str((currentsegmentsize + len(segment))))
    ...
    while len(segment) > target * tolerancefactor:
        # print(relname + "\t Last segment size: " + str(currentsegmentsize) +
        # "\t appending " + str(target) + "\t for a total of " +
        # str((currentsegmentsize + target)))
    ...
    # if the remaining part is exceeding current segment's capacity start new
    # segment
    if currentsegmentsize + len(segment) > target * tolerancefactor:
        # segment is filled. continue with next one
        counter += 1
...
def segmenter(inpath, outfolder, target, sizetolerancefactor, preserveparagraphs=False):
    """
    Turns plain text files into equal-sized segments, with limited respect for
    paragraph boundaries.

    output files are named after the fileneme of the text file with a 6 digit
    extension.
    output files will be saved at directory "outfolder"
    tokenizes texfiles and deletes whitespacethe by line
    calls writesegment(words, outfolder, filename, target, sizetolerancefactor,
    preserveparagraphs)

    Args:
        inpath(str): directory of the folder containing text files to be
            segmented
    ...
    for relfile in glob.glob(inpath):
        ...
        with open(file, "r") as infile:
            ...
            # creates 6digit file name extension
            segname = join(outfolder, filename +
                           "§{:06d}".format(counter) + ".txt")
            ...
            for line in infile:
                text = line
                # (optional) remove punctuation, special characters and
                # space-chains
                # text = re.sub("[,;\.:!?¿\(\)—-]", " ", text)
                text = re.sub("[\t\r\n\v\f]", " ", text)
                ...
                words.append("\n")
                writesegment(words, outfolder, filename, target,
                             sizetolerancefactor, preserveparagraphs)
    print("Done.")
...
def segments_to_bins(inpath, outfolder, binsnb):
    ...
    # used to track wrong segmenting (i.e. with segment numbering not starting
    # with 0)
    offset = sys.maxsize
    ...
    # For each filename, compute and append bin number
        ...
        # avoid 1.0 beeing in seperate bin (should never happen due to offset!)
        if binid == binsnb:
...
def call_treetagger(infolder, outfolder, tagger):
    ...
    for infile in infiles:
        ...
        outfile = os.path.join(
            outfolder, os.path.basename(infile)[:-4] + ".trt")
        # print(outfile)
                ...
                if len(splitline) == 3:
                    ...
                    if mode == "frN":
                        ...
                        if "Nc" in pos and "|" not in lemma and "<unknown>" not in lemma:
                            ...
                            lemmata.append(token.lower())
            # Continue with list of lemmata, but remove undesired leftover
            # words
            lemmata = ' '.join(
                [word for word in lemmata if word not in stoplist])
